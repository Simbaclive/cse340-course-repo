...
const createProject = async (organizationId, title, description, location, date) => {
    ...
    const result = await db.query(query, [organizationId, title, description, location, date]);
    return result.rows[0];
};

const updateProject = async (id, organizationId, title, description, location, date) => {
    const query = `
        UPDATE public.project
        SET organization_id = $1, title = $2, description = $3, location = $4, date = $5
        WHERE project_id = $6 RETURNING *`;
    const result = await db.query(query, [organizationId, title, description, location, date, id]);
    return result.rows[0];
};
